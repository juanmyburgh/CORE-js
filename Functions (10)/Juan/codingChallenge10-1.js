...
const poll = {
    ...
    registerNewAnswer() {
        ...
        // Task 1.2
        typeof answer === 'number' && answer < this.options.length && answer >= 0 ? this.answers[answer]++ : console.log(`${answer} is not a valid option!`);
        // typeof answer === 'number' && answer < this.options.length && answer >= 0 && this.answers++; // this could've been a shortcircuit

        // Task 4
        this.displayResults(prompt('What result format?'), this.answers);
    },
...
// these are wrong, shouldn't update the method to account for array
// poll.displayResults('array', bonusData1);
// poll.displayResults('string', bonusData1);
// poll.displayResults('array', bonusData2);
// poll.displayResults('string', bonusData2);
// instead:
poll.displayResults.call({ answers: bonusData1 });
poll.displayResults.call({ answers: bonusData1 }, 'string');
poll.displayResults.call({ answers: bonusData2 });
